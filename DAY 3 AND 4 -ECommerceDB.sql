...
SELECT COUNT(sale_id) AS Total_no_of_salesrecords
FROM sales;
SELECT SUM(sale_amount) as total_sales_amount
FROM sales;
SELECT AVG(sale_amount) as average_sales_amount
FROM sales;
SELECT MAX(sale_amount) as max_sales_amount
FROM sales;
SELECT MIN(sale_amount) as min_sales_amount
FROM sales;
SELECT product_id, 
...
FROM sales
GROUP BY product_id
HAVING SUM(sale_amount) >30000 AND AVG(sale_amount) > 10000 ;
